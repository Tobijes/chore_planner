...
function write_excel(x, filepath)
    XLSX.openxlsx(filepath, mode="w") do xf
        sheet = xf[1]
        XLSX.rename!(sheet, "Pligtplan")
        sheet["A1"] = vcat(["Uge"], users)
        sheet["A2", dim=1] = collect(P)

        for p in P
            for u in U
                active_task_indexes = findall(x -> x == 1, x[u,p,:])
                active_task_labels = map(i -> tasks[i].label, active_task_indexes)
                joined_labels = join(active_task_labels, "\r\n")
                cell = Char(Int('A') + u) * string(1 + p)
                sheet[cell] = joined_labels
            end
...
function plot_weekly_workload(w)

    StatsPlots.groupedbar(
        w',
        bar_position = :group,
        labels = reshape(users, (1, length(users))),
        xlabel = "Uge",
        ylabel = "Arbejdstid [minutter]",
        xticks=P,
        bar_width = 0.65
    )
end
...
write_excel(x_sol, "solution.xlsx")

w_sol = Array(value.(w))
plot_weekly_workload(w_sol)
